...
int main(int argc, char** argv)
{
    ...
    //ustawianie listenera na konczenie procesu dzieci
    sethandler(sigchld_handler, SIGCHLD);
    //ustawianie listenera na SIGUSR1, SIGUSR2
    sethandler(sig_handler, SIGUSR1);
    sethandler(sig_handler, SIGUSR2);
    // PODCZAS TEGO PROCESU USTAWIANY JEST TYLKO SPOSOB HANDLOWANIA
    // SYGNALOW KTORE W PRZYSZLOSCI DOTRA, GDY DOTRZE ODPOWIEDNI
    // SYGNAL ZMIENNA last_signal USTAWI SIE NA NUMER TEGO SYGNALU I
    // FUNKCJA PODANA W PIERWSZYM ARGUMENCIE ZACZNIE SIE WYWOLYWAC

    sigset_t mask, oldmask;
    sigemptyset(&mask);
    // dodawanie sygnalow do sigseta
    sigaddset(&mask, SIGUSR1); 
    sigaddset(&mask, SIGUSR2);
    // dodawanie sigseta do sigmaski programowej
