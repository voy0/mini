...
{
    ssize_t c; // signed int type [-1, SSIZE_MAX]
    ssize_t len = 0;
    do
    {
        // odczytuje dane i zapisuje do bufora
        c = TEMP_FAILURE_RETRY(read(fd,buf,count));
        if(c < 0)
            return c;// nie mozna bylo huja przeczytac i sie poddal
        if(c == 0)
            return len;// EOF
        buf += c; // przesuwa bufor o ilosc przecytanych bajtow
        len += c; // zlicza wielkosc pliku
        count -= c; // ile zostalo
    }while(count > 0); // powtarzaj dopoki nie przeczytano wszystkiego
    return len;
}
